...
TreeGrid = function(_config){
	...
	//显示表头行
	drowHeader = function(){
		s += "<thead><tr  height='" + (_config.headerHeight || "25") + "'>";
		var cols = _config.columns;
		for(i=0;i<cols.length;i++){
			var col = cols[i];
			s += "<th>" + (col.headerText || "") + "</th>";
		}
		s += "</tr></thead>";
	}
			...
			for(var j=0;j<_cols.length;j++){
				...
				if(j==folderColumnIndex){
					...
					if(row.children){ //有下级数据
						s += "<span folder='Y' trid='TR" + id + "'  class='image_hand "+row.css_icon+"'></span> ";
					}else{
						s += "<span  class='image_nohand  "+row.css_icon+"'></span> ";
					}
				}
	...
	this.show = function(){
		...
		s += "<table id='" + this.id + "'  class='table table-bordered table-striped'>";
		drowHeader();
		drowData();
		s += "</table>";
	...
	showHiddenNode = function(_trid, _open){
		if(_open == "N"){ //隐藏子节点
			__root.find("#"+_trid).find("span[folder='Y']").attr("src", folderCloseIcon);
...
//公共静态变量
TreeGrid.FOLDER_OPEN_ICON = "icon-plus-circle2";
TreeGrid.FOLDER_CLOSE_ICON = "icon-minus-circle2";
